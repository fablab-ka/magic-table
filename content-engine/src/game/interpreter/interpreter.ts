import { executeCodeCommand } from "../state/actions";
import {
  Context,
  InterpreterError,
  ProgramList,
...
export default class Interpreter {
  ...
  private dispatch: (action: ReturnType<typeof executeCodeCommand>) => void;

  constructor(
    dispatch: (action: ReturnType<typeof executeCodeCommand>) => void
  ) {
    this.programs = [];
    this.callstack = [];
    this.contextStack = [];
  ...
  public updateTokens(tokenLists: Token[][]) {
    ...
    tokenLists.forEach((tokens, index) => {
      ...
      if (tokens[0].type === TokenType.Start) {
        ...
        programList[
          tokens[0].parameters[0].value as ProgramSlot
        ] = tokens.slice(1);
      } else {
  ...
  private doesCloseContext(token: Token): boolean {
    return (
      token.type === TokenType.End || token.type === TokenType.ConditionElse
    );
  }
  ...
  private eat(token: Token, context?: Context) {
    ...
    if (token.type === TokenType.MethodCall) {
      ...
      this.dispatch(executeCodeCommand(token.parameters[0].value));
    } else {
